    ...
    if (isset($_POST['action_mes']) && $_POST['action_mes'] == 'delete') {
        ...
        if (isset($slider_id) && !empty($slider_id)) {
            if ($_FILES["slider_u"]["name"] == '') {
                ...
                slider_status = '".$_POST['slider_status']."', slider_type ='".$_POST['slider_type']."' WHERE slider_id='$slider_id'";
                $res = pg_query($db, $sql);
                if($ret != ''){
                    echo 7;
                }
            ...
            if (! in_array($FileType, $allowed_image_extension)) {
                ...
                if (move_uploaded_file($_FILES["slider_upload"]["tmp_name"], $target_dir.$name.$_FILES["slider_upload"]["name"])) {
                    ...
                    ('$slider_name','$target_file','$slider_caption','".$_POST['slider_status']."','".$_POST['slider_type']."')";
                    $ret = pg_query($db, $sql);
                    if($ret != ''){
                        echo 1;
                    }
