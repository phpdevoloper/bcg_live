<?php include('inc/header.php'); 
include('inc/dbconnection.php');
?>
<!-- Home -->
<!-- <div class="banner">
    <img src="images/about.jpg" alt="" />
</div> -->
<!-- product description -->
<div class="about">
    <section>
        <ul class="breadcrumb wizard">
            <li class="completed"><a href="index.php"><i class="fa fa-home"></i></a></li>
            <li class="completed"><a href="javascript:void(0);">Division</a></li>
            <li class=""><a href="administration.php">FY 2020-2021</a></li>
        </ul>
    </section>
    <div class="section">
        <h3 class="text-center txt" style="color: #299adc;">GMSD and State-wise supply of BCG Vaccine in FY 2021-22</h3>
    </div>
    <div class="container aos-init aos-animate" data-aos="fade-up" style="padding-top: 38px;">
        <div class="container">
            <!-- About Content -->
            <section id="about">
                <div class="container aos-init aos-animate" data-aos="fade-up">
                    <p>&nbsp;<img src="images/bcgvl_Vaccine supply details for FY 2021-22.png" alt="" width="100%"></p>
                </div>
            </section>
        </div>
    </div>
</div>
<?php include('inc/simple_footer.php'); ?>