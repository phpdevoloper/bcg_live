$(document).ready(function () {
  ...
  $("#edit_mst_rti1").submit(function (e) {
    e.preventDefault();
    var data = new FormData(this);
    swal({
      title: "Are you sure?",
  ...
  $("#edit_mst_rti2").submit(function (e) {
    e.preventDefault();
    var data = new FormData(this);
    swal({
      title: "Are you sure?",
  ...
  $("#edit_mst_rti3").submit(function (e) {
    e.preventDefault();
    var data = new FormData(this);
    swal({
      title: "Are you sure?",
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (result.code == "200") {
              ...
              swal({
                title: "Something went wrong in the Database",
                icon: "error",
              });
            }
          },
  ...
  // Add Sub RTI
  $("#add_sub_rti").submit(function (e) {
    e.preventDefault();
    var data = new FormData(this);
    data.append("action_mes","Sub_RTI_INSERT");
    swal({
      title: "Are you sure?",
      text: "You wants to add RTI!",
      icon: "warning",
      buttons: ["No, cancel it!", "Yes, I am sure!"],
      dangerMode: true,
    }).then(function (isConfirm) {
  ...
  $(document).on("click", ".getSub_RTI", function (e) {
    ...
    var rti_upload  = $(this).attr("data-rti_upload");
    var action      = $(this).attr("data-action");
    // console.log(file_type);

    // $("#Edit_Rti_id3").val(subs_id);
    // $("#Edit_rti_title3").val(subs_name);

    $("#Edit_rti_title3").val(subs_name);
    $("#Edit_Rti_id3").val(subs_id);
    if(file_type == 'Url'){
      $('#RTI_url_check3').prop("checked", true);
      $('#RTI_url3').attr("style", "display:block");
      $('#RTI_url3').val(rti_upload);
    }else{
      $('#RTI_file_check3').prop("checked", true);
      $('#RTI_file3').attr("style", "display:block");
      $('#file_name3').text(rti_upload);
    }
    $("#action3").val(action);
  });

  $(document).on("click", ".getSubLone_RTI", function (e) {
    var subs_id = $(this).attr("data-sub_id");
    var subs_name = $(this).attr("data-sub_name");
    var action = $(this).attr("data-action");
    // console.log(subs_id);

    $("#Edit_rti_title1").val(subs_name);
    $("#Edit_Rti_id1").val(subs_id);
    $("#action1").val(action);
  });

  $(document).on("click", ".getLTWORTI", function (e) {
    var subs_id     = $(this).attr("data-subs_id");
    var subs_name   = $(this).attr("data-subs_name");
    var file_type   = $(this).attr("data-file_type");
    var rti_upload  = $(this).attr("data-rti_upload");
    ...
    $("#Edit_rti_title2").val(subs_name);
    $("#Edit_Rti_id2").val(subs_id);
    if(file_type == 'Url'){
      $('#RTI_url_check2').prop("checked", true);
      $('#RTI_url2').attr("style", "display:block");
      $('#RTI_url2').val(rti_upload);
    }else{
      $('#RTI_file_check2').prop("checked", true);
      $('#RTI_file2').attr("style", "display:block");
      $('#file_name2').text(rti_upload);
    }
    $("#action2").val(action);
  });
