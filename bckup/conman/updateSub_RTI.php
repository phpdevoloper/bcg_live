<?php
	...
	// 	}else{
            if ($_FILES["sub_rti_file"]["name"] !== '') {
                ...
                $name =date('m-d-Y_');
                $target_file = $name.basename($_FILES["sub_rti_file"]["name"]);
                $FileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
                // var_dump($imageFileType);die;
                
                // Check if image file is a actual image or fake image
                if (move_uploaded_file($_FILES["sub_rti_file"]["tmp_name"], $target_dir.$name.$_FILES["sub_rti_file"]["name"])) {
                    $query = "UPDATE mst_sub_rti SET file_type ='$FileType',
                    rti_upload ='$target_file' WHERE sub_rti_id = '".$_POST['sub_rti_id']."'";
                ...
                // var_dump($_POST);die;
                $FileType = 'Url';
                $query = "UPDATE mst_sub_rti SET  rti_upload='".$_POST['sub_rti_url']."',file_type ='$FileType' WHERE sub_rti_id = '".$_POST['sub_rti_id']."'";
                $ret = pg_query($db, $query);
                echo 1;
            }
			pg_close($db);
